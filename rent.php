		...
		if(isset($_GET['delete']) && isset($_GET['confirm']))
				{
					...
					if($_GET['confirm'] == 'yes')
						...
						$sql5_revenue = $dbhi->prepare("SELECT token_id FROM rent WHERE id='$delete_rent'");
							...
							$sql5 = $dbhi->prepare("DELETE from rent WHERE id=$delete_rent");
							$sql5->execute();
					...
					<div class="portlet box yellow">
						<div class="portlet-title">
							<div class="caption">
								...
								<i class="fa fa-user"></i>Rent Account 
						...
						?>
							...
							<div align="right" style="padding:12px; font-weight:none; font-size:17px;"><a href="add_rent.php">Add Rent</a></div>
						</div>
...
$delete_rent=$_GET['delete'];
{
							...
							$sql4 = $dbhi->prepare("SELECT * FROM  rent WHERE id=$delete_rent");
...
$get_edit_id=$_GET['edit'];
	header("location:edit_rent.php?id=$get_edit_id");
?>
		...
		<form name="edit_rent" id="edit_rent" method="POST" novalidate="novalidate" action="<?php  echo $_SERVER['PHP_SELF']."?edit=".$key3['id'];?>">
<?php		


$get_edit=$_GET['edit'];
$sql2 = $dbhi->prepare("SELECT * FROM  rent WHERE id=$get_edit");
	$sql2->execute();
...
if(!isset($_GET['delete']) && !isset($_GET['edit']))
{
					...
					$sql2 = $dbhi->prepare("SELECT * FROM  rent WHERE date BETWEEN '$from_date' AND '$to_date'  ORDER BY `id` DESC");
							}
							else
							{
							$sql2 = $dbhi->prepare("SELECT * FROM  rent  ORDER BY `id` DESC");
							}		
							$sql2->execute();
