...
define("DB_PASSWORD","");
		...
		if(isset($_GET['delete']) && isset($_GET['confirm']))
				{
					$delete_insurance=$_GET['delete'];
					if($_GET['confirm'] == 'yes')
						{
							$sql5 = $dbhi->prepare("DELETE from insurance WHERE id=$delete_insurance");
							$sql5->execute();
							header("location:insurance_payment.php?data_succesfully_deleted");
					...
					<div class="portlet box yellow">
						<div class="portlet-title">
							<div class="caption">
								...
								<i class="fa fa-user"></i>Insurance Account 
						...
						$xyz=datamanager::get_sum_tot('insurance','amount_paid','pric');
						...
						?>
							...
							<div align="right" style="padding:12px; font-weight:none; font-size:17px;"><a href="add_insurance.php">Add Insurance </a></div>
						</div>
...
$delete_insurance=$_GET['delete'];
{
							...
							$sql4 = $dbhi->prepare("SELECT * FROM  insurance WHERE id=$delete_insurance");
							...
							for($i=0; $key3 = $sql4->fetch(); $i++)
						{
							...
							<!--<td><input type="checkbox" class="checkboxes" value="<?php echo $key3['id'];?>"/></td>-->
								...
								<td><?php echo $key3['id'];?></td>
                                <td><?php echo $key3['insurance_company'];?></td>
                                <td><?php echo $key3['agent'];?></td>
...
$get_edit_id=$_GET['edit'];
	header("location:edit_insurance.php?id=$get_edit_id");
?>
		...
		<form name="edit_insurance" id="edit_insurance" method="POST" novalidate="novalidate" action="<?php  echo $_SERVER['PHP_SELF']."?edit=".$key3['id'];?>">
<?php		


$get_edit=$_GET['edit'];
$sql2 = $dbhi->prepare("SELECT * FROM  insurance WHERE id=$get_edit");
	$sql2->execute();
						...
						?>	
							...
							<tr class="odd gradeX">
								...
								<td><input class="new_input_text" type="text" name="insurance_company" value="<?php 
								if($_POST['insurance_company'])
								{
								echo 'test';
								}
								echo $key3['insurance_company'];
								?>"/></td>
								<td><input class="new_input_text" type="text" name="agent" value="<?php echo $key3['agent'];?>"/></td>
...
if(!isset($_GET['delete']) && !isset($_GET['edit']))
{
					...
					$sql2 = $dbhi->prepare("SELECT * FROM  insurance WHERE date BETWEEN '$from_date' AND '$to_date'  ORDER BY `id` DESC");
							}
							else
							{
							$sql2 = $dbhi->prepare("SELECT * FROM  insurance  ORDER BY `id` DESC");
							}		
							$sql2->execute();
