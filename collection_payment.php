...
define("DB_PASSWORD","");
		...
		if(isset($_GET['delete']) && isset($_GET['confirm']))
				{
					$delete_collection1=$_GET['delete'];
					if($_GET['confirm'] == 'yes')
						{
						$sql5_revenue = $dbhi->prepare("SELECT token_id FROM collection1 WHERE id='$delete_collection1'");
							...
							$sql5 = $dbhi->prepare("DELETE from collection1 WHERE id=$delete_collection1");
							$sql5->execute();
							header("location:collection_payment.php?data_succesfully_deleted");
					...
					<div class="portlet box yellow">
						<div class="portlet-title">
							<div class="caption">
								...
								<i class="fa fa-user"></i>Collection Payment Account 
						...
						$xyz=datamanager::get_sum_tot('collection1','amount','pric');
						...
						?>
							...
							<div align="right" style="padding:12px; font-weight:none; font-size:17px;"><a href="add_collection1.php">Add Collection Payment</a></div>
						</div>
...
$delete_collection1=$_GET['delete'];
{
							...
							$sql4 = $dbhi->prepare("SELECT * FROM  collection1 WHERE id=$delete_collection1");
...
$get_edit_id=$_GET['edit'];
	header("location:edit_collection1.php?id=$get_edit_id");
?>
		...
		<form name="edit_collection1" id="edit_collection1" method="POST" novalidate="novalidate" action="<?php  echo $_SERVER['PHP_SELF']."?edit=".$key3['id'];?>">
<?php		


$get_edit=$_GET['edit'];
$sql2 = $dbhi->prepare("SELECT * FROM  collection1 WHERE id=$get_edit");
	$sql2->execute();
...
if(!isset($_GET['delete']) && !isset($_GET['edit']))
{
					...
					$sql2 = $dbhi->prepare("SELECT * FROM  collection1 WHERE date BETWEEN '$from_date' AND '$to_date'  ORDER BY `id` DESC");
							}
							else
							{
							$sql2 = $dbhi->prepare("SELECT * FROM  collection1  ORDER BY `id` DESC");
							}		
							$sql2->execute();
