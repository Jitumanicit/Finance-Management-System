...
<?php require_once(dirname(__FILE__).'/sidebar.php');?>	
	...
	$get_id=$_GET['id'];
$mysql_select_dealer = $dbhi2->prepare("SELECT * FROM `finance`.`interest_payment` WHERE `id`=$get_id");
...
$db_dealer_value = $mysql_select_dealer->fetch();
			...
			<div class="portlet box yellow">
									<div class="portlet-title">
										<div class="caption">
											<i class="fa fa-gift"></i>Edit Interest Payment
										</div>										
									</div>
									<div class="portlet-body form">
										<!-- BEGIN FORM-->
...
<input value="<?php echo $db_dealer_value['amount']; ?>" type="text" name="amount" class="form-control" placeholder="Enter Amount">														
											...
											<div class="form-body">
												<div class="form-group">
													...
													<div class="col-md-4">
<input value="<?php echo $db_dealer_value['month']; ?>" type="text"  name="month" class="form-control" placeholder="Enter Month">														
													</div>
												</div>
											</div>																
											<div class="form-body">
...
<input value="<?php echo $db_dealer_value['date']; ?>" type="date"  name="date" class="form-control" placeholder="Enter Date">																									
	...
	*/
$mysql_update_dealer = $dbhi2->prepare("UPDATE `finance`.`interest_payment` 
SET `amount`='$amount', 
	`month`='$month', 
	`date`='$date' 
WHERE  `id`=$get_id;");
...
WHERE  `token_id`='$revenue_office_exp_token';");
	$mysql_update_revenue_office_exp->execute();
							...
							header("location:interest_payment.php?show_id=$get_id&update=data_sucessfully_updated");
