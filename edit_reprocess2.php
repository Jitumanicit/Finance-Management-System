...
<?php require_once(dirname(__FILE__).'/sidebar.php');?>	
	...
	$get_id=$_GET['id'];
$mysql_select_dealer = $dbhi2->prepare("SELECT * FROM `finance`.`reprocess2` WHERE `id`=$get_id");
...
$db_dealer_value = $mysql_select_dealer->fetch();
			...
			<div class="portlet box yellow">
									<div class="portlet-title">
										<div class="caption">
											<i class="fa fa-gift"></i>Edit Reprocess
										</div>										
									</div>
									<div class="portlet-body form">
										<!-- BEGIN FORM-->
...
<input value="<?php echo $db_dealer_value['date']; ?>" type="text"  name="date" class="form-control" placeholder="Enter Date">														
                                            ...
                                            <div class="form-body">
												<div class="form-group">
													...
													<div class="col-md-4">
<input value="<?php echo $db_dealer_value['amount']; ?>" type="text"  name="amount" class="form-control" placeholder="Enter Amount Paid">														
													</div>
												</div>
											</div>
                                            <div class="form-body">
...
<input value="<?php echo $db_dealer_value['client_name']; ?>" type="text"  name="client_name" class="form-control" placeholder="Enter Client Name">																									
									...
									<button type="submit" class="btn green"	onclick="this.value='Submitting ..';this.disabled='disabled'; this.form.submit();">Update</button>																		  									<!--<button type="button" class="btn default">Cancel</button>-->
														<a href="reprocess2.php" class="btn red">Cancel</a>
													</div>
												</div>
											</div>
										</form>
	...
	*/
$mysql_update_dealer = $dbhi2->prepare("UPDATE `finance`.`reprocess2` 
SET `name`='$name',
	`date`='$date',
	`amount`='$amount', 
...
WHERE  `token_id`='$revenue_office_exp_token';");
	$mysql_update_revenue_office_exp->execute();
							...
							header("location:reprocess2.php?show_id=$get_id&update=data_sucessfully_updated");
