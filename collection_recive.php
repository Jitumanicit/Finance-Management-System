					...
					if($_GET['confirm'] == 'yes')
						{
							...
							$sql5_revenue = $dbhi->prepare("SELECT token_id FROM collection WHERE id='$delete_collection'");
							...
							$sql5 = $dbhi->prepare("DELETE from collection WHERE id=$delete_collection");
							$sql5->execute();
							header("location:collection_recive.php?data_succesfully_deleted");
					...
					<div class="portlet box yellow">
						<div class="portlet-title">
							<div class="caption">
								...
								<i class="fa fa-user"></i>Collection Receive Account 
						...
						$xyz=datamanager::get_sum_tot('collection','amount','pric');
						...
						?>
							...
							<div align="right" style="padding:12px; font-weight:none; font-size:17px;"><a href="add_collection.php">Add Collection Receive</a></div>
						</div>
...
$delete_collection=$_GET['delete'];
{
							...
							$sql4 = $dbhi->prepare("SELECT * FROM  collection WHERE id=$delete_collection");
...
$get_edit_id=$_GET['edit'];
	header("location:edit_collection.php?id=$get_edit_id");
?>
		...
		<form name="edit_collection" id="edit_collection" method="POST" novalidate="novalidate" action="<?php  echo $_SERVER['PHP_SELF']."?edit=".$key3['id'];?>">
<?php		


$get_edit=$_GET['edit'];
$sql2 = $dbhi->prepare("SELECT * FROM  collection WHERE id=$get_edit");
	$sql2->execute();
...
if(!isset($_GET['delete']) && !isset($_GET['edit']))
{
					...
					$sql2 = $dbhi->prepare("SELECT * FROM  collection WHERE date BETWEEN '$from_date' AND '$to_date'  ORDER BY `id` DESC");
							}
							else
							{
							$sql2 = $dbhi->prepare("SELECT * FROM  collection  ORDER BY `id` DESC");
							}		
							$sql2->execute();
