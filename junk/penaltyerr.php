...
datamanager::save( array('penalty' => $wwe, 'update_time' => $update_time, 'status' => 0) , 'emi_table' ,  array('id' => $key['id'] ) );									
								...
								//$data_count = datamanager::get_all('client');
						//foreach ($data_count as $key_array) {
									//$count_d=$key_array['penalty_lapse_count']+1;
									//if(isset($key_array['id'])
									//{
										//datamanager::save( array('penalty_lapse_count' => $count_d) , array('id' => $key['id']), 'client'); 
									//}
															} 
