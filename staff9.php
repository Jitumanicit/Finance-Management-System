...
define("DB_PASSWORD","");
		...
		if(isset($_GET['delete']) && isset($_GET['confirm']))
				{
					$delete_staff9=$_GET['delete'];
					if($_GET['confirm'] == 'yes')
						{
							$sql5 = $dbhi->prepare("DELETE from staff9 WHERE id=$delete_staff9");
							$sql5->execute();
							header("location:staff9.php?data_succesfully_deleted");
			...
			<div class="row">
				<div class="col-md-12 col-sm-12">					
					<div class="portlet box yellow">
						...
						$xyz=datamanager::get_sum_tot('staff9','salary','pric');
						foreach ($xyz as $keyxyz);
						echo 'Total='.$keyxyz['pric'].'&nbsp;&nbsp;&nbsp;';
						?>
						
						</div>
							<div align="right" style="padding:12px; font-weight:none; font-size:17px;"><a href="salary9.php">Add Salary</a></div>
						</div>
...
$delete_staff9=$_GET['delete'];
{
							...
							$sql4 = $dbhi->prepare("SELECT * FROM  staff9 WHERE id=$delete_staff9");
...
$get_edit_id=$_GET['edit'];
	header("location:edit_staff9.php?id=$get_edit_id");
?>
		...
		<form name="edit_staff9" id="edit_staff9" method="POST" novalidate="novalidate" action="<?php  echo $_SERVER['PHP_SELF']."?edit=".$key3['id'];?>">
<?php		


$get_edit=$_GET['edit'];
$sql2 = $dbhi->prepare("SELECT * FROM  staff9 WHERE id=$get_edit");
	$sql2->execute();
...
if(!isset($_GET['delete']) && !isset($_GET['edit']))
{
					...
					$sql2 = $dbhi->prepare("SELECT * FROM  staff9 WHERE date BETWEEN '$from_date' AND '$to_date'  ORDER BY `id` DESC");
							}
							else
							{
							$sql2 = $dbhi->prepare("SELECT * FROM  staff9  ORDER BY `id` DESC");
							}		
							$sql2->execute();
