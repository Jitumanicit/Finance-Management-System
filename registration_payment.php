...
define("DB_PASSWORD","");
		...
		if(isset($_GET['delete']) && isset($_GET['confirm']))
				{
					$delete_registration=$_GET['delete'];
					if($_GET['confirm'] == 'yes')
						{
							$sql5 = $dbhi->prepare("DELETE from registration WHERE id=$delete_registration");
							$sql5->execute();
							header("location:registration_payment.php?data_succesfully_deleted");
					...
					<div class="portlet box yellow">
						<div class="portlet-title">
							<div class="caption">
								...
								<i class="fa fa-user"></i>Registration Account 
						...
						$xyz=datamanager::get_sum_tot('registration','amount','pric');
						...
						?>
							...
							<div align="right" style="padding:12px; font-weight:none; font-size:17px;"><a href="new registration payment.php">Add Registration</a></div>
						</div>
...
$delete_registration=$_GET['delete'];
{
							...
							$sql4 = $dbhi->prepare("SELECT * FROM  registration WHERE id=$delete_registration");
...
$get_edit_id=$_GET['edit'];
	header("location:edit_registration.php?id=$get_edit_id");
?>
		...
		<form name="edit_registration" id="edit_registration" method="POST" novalidate="novalidate" action="<?php  echo $_SERVER['PHP_SELF']."?edit=".$key3['id'];?>">
<?php		


$get_edit=$_GET['edit'];
$sql2 = $dbhi->prepare("SELECT * FROM  registration WHERE id=$get_edit");
	$sql2->execute();
						...
						?>	
							...
							<tr class="odd gradeX">
								...
								<td><input class="new_input_text" type="text" name="client_name" value="<?php 
								if($_POST['client_name'])
								{
								echo 'test';
								}
								echo $key3['client_name'];
								?>"/></td>
...
if(!isset($_GET['delete']) && !isset($_GET['edit']))
{
					...
					$sql2 = $dbhi->prepare("SELECT * FROM  registration WHERE date BETWEEN '$from_date' AND '$to_date'  ORDER BY `id` DESC");
							}
							else
							{
							$sql2 = $dbhi->prepare("SELECT * FROM  registration  ORDER BY `id` DESC");
							}		
							$sql2->execute();
