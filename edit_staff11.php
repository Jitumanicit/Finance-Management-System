...
<?php require_once(dirname(__FILE__).'/sidebar.php');?>	
	...
	$get_id=$_GET['id'];
	$mysql_select_dealer = $dbhi2->prepare("SELECT * FROM `finance`.`staff11` WHERE `id`=$get_id");
...
<input value="<?php echo $db_dealer_value['date']; ?>" type="date"  name="date" class="form-control" placeholder="Enter Date">														
									...
									<button type="submit" class="btn green"	onclick="this.value='Submitting ..';this.disabled='disabled'; this.form.submit();">Update</button>																		  									<!--<button type="button" class="btn default">Cancel</button>-->
														<a href="staff11.php" class="btn default">Cancel</a>
													</div>
												</div>
											</div>
										</form>
	...
	*/
$mysql_update_dealer = $dbhi2->prepare("UPDATE `finance`.`staff11` 
SET `name`='$name',
	`role`='$role',
	`salary`='$salary', 
	`advance`='$advance', 
...
WHERE  `id`=$get_id;");
	$mysql_update_dealer->execute();
							...
							header("location:staff11.php?show_id=$get_id&update=data_sucessfully_updated");
