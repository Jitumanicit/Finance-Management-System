...
define("DB_PASSWORD","");
		...
		if(isset($_GET['delete']) && isset($_GET['confirm']))
				{
					...
					if($_GET['confirm'] == 'yes')
						{
							$sql5 = $dbhi->prepare("DELETE from remunerationn WHERE id=$delete_staff");
							$sql5->execute();
							header("location:remuneration.php?data_succesfully_deleted");
					...
					<div class="portlet box yellow">
						<div class="portlet-title">
							<div class="caption">
								...
								<i class="fa fa-user"></i>Partner's List 
	...
	public static function xxxxx($condition = NULL){
		$db= new database;
			if($condition){
				return $db->xxxxx($condition);		
			}else{
				$sql="SELECT SUM(tapan.withdraw_total) AS withdraw_total
  FROM ( SELECT SUM(withdraw) AS withdraw_total
           FROM rm1
          UNION ALL
         SELECT SUM(withdraw) AS withdraw_total
           FROM rm2
		   UNION ALL
         SELECT SUM(withdraw) AS withdraw_total
           FROM rm3 
         ...
         SELECT SUM(withdraw) AS withdraw_total
           FROM rm6
				...
				return $db->query($sql);	
						...
						echo '<b>Total='.$keyxyz['withdraw_total'].'&nbsp;&nbsp;&nbsp;</b>';
						?>
						
						</div>
							
							
							<div align="right" style="padding:12px; font-weight:none; font-size:17px;"><a href="add_remuneration.php">Add Partner</a></div>
						</div>
...
$delete_staff=$_GET['delete'];
{
							...
							$sql4 = $dbhi->prepare("SELECT * FROM  remunerationn WHERE id=$delete_staff");
							...
							<!--<td><input type="checkbox" class="checkboxes" value="<?php echo $key3['id'];?>"/></td>-->
                                ...
                                <?php if($key3['id']==1) { ?>
									...
									<div class="btn-toolbar margin-bottom-10">
                                                       ...
                                                       <a class="btn btn-circle btn-sm blue-stripe" href="remunerationn.php.php?id=<?=$key['id'];?>" >VIEW<span class="glyphicon glyphicon-user"></span></a>
														</div>
													</div>
...
$get_edit_id=$_GET['edit'];
	header("location:edit_remuneration.php?id=$get_edit_id");
?>
		...
		<form name="edit_remuneration" id="edit_remuneration" method="POST" novalidate="novalidate" action="<?php  echo $_SERVER['PHP_SELF']."?edit=".$key3['id'];?>">
<?php		


$get_edit=$_GET['edit'];
$sql2 = $dbhi->prepare("SELECT * FROM  remunerationn WHERE id=$get_edit");
	$sql2->execute();
						...
						?>	
							...
							<tr class="odd gradeX">
								...
								echo $key3['name'];
								?>"/></td>
								<td><input class="new_input_text" type="text" name="email" value="<?php echo $key3['phone'];?>"/></td>
                                <td><input class="new_input_text" type="text" name="phone" value="<?php echo $key3['email'];?>"/></td>
								<td><input class="new_input_text" type="text" name="role" value="<?php echo $key3['address'];?>"/></td>
   								<td><input class="new_input_text" type="text" name="role" value="<?php echo $key3['salary'];?>"/></td>
								<!-- <td> -->
								<td>
								<div class="btn-toolbar margin-bottom-10">
														<div class="btn-group btn-group-sm btn-group-solid">
...
if($key3['id']==1)
	header("location:rm1.php?id=$get_view_id");
	if($key3['id']==2)
	header("location:rm2.php?id=$get_view_id");
	if($key3['id']==3)
	header("location:rm3.php?id=$get_view_id");
	if($key3['id']==4)
	header("location:rm4.php?id=$get_view_id");
	if($key3['id']==5)
	header("location:rm5.php?id=$get_view_id");
	if($key3['id']==6)
	header("location:rm6.php?id=$get_view_id");
...
if(!isset($_GET['delete']) && !isset($_GET['edit']) && !isset($_GET['view']))
{
							...
							$sql2 = $dbhi->prepare("SELECT * FROM remunerationn ORDER BY `id` DESC");
							...
							for($i=0; $key3 = $sql2->fetch(); $i++)
						{
							...
							<tr class="odd gradeX">
                                ...
                                <div class="btn-toolbar margin-bottom-10">
														<div class="btn-group btn-group-sm btn-group-solid">
                                                       <a class="btn btn-circle btn-sm blue-stripe" href="<?php echo "rm".$key3['id'].".".php;?>" >VIEW<span class="glyphicon glyphicon-user"></span></a>
														</div>
													</div>
