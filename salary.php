...
define("DB_PASSWORD","");
		...
		if(isset($_GET['delete']) && isset($_GET['confirm']))
				{
					$delete_salary=$_GET['delete'];
					if($_GET['confirm'] == 'yes')
						{
							$sql5 = $dbhi->prepare("DELETE from salary WHERE id=$delete_salary");
							$sql5->execute();
							header("location:salary.php?data_succesfully_deleted");
				...
				<div class="col-md-12 col-sm-12">					
					<div class="portlet box yellow">
						<div class="portlet-title">
							...
							<div align="right" style="padding:12px; font-weight:none; font-size:17px;"><a href="add_salary.php">Add Salary</a></div>
						</div>
...
$delete_salary=$_GET['delete'];
{
							...
							$sql4 = $dbhi->prepare("SELECT * FROM  salary WHERE id=$delete_salary");
...
$get_edit_id=$_GET['edit'];
	header("location:edit_salary.php?id=$get_edit_id");
?>
		...
		<form name="edit_salary" id="edit_salary" method="POST" novalidate="novalidate" action="<?php  echo $_SERVER['PHP_SELF']."?edit=".$key3['id'];?>">
<?php		


$get_edit=$_GET['edit'];
$sql2 = $dbhi->prepare("SELECT * FROM  salary WHERE id=$get_edit");
	$sql2->execute();
...
if(!isset($_GET['delete']) && !isset($_GET['edit']))
{
							...
							$sql2 = $dbhi->prepare("SELECT * FROM  salary  ORDER BY `id` DESC");
...
<td><a class="btn btn-circle btn-sm blue-stripe" href="salary_st.php?id=<?=$key['id'];?>" >VIEW<span class="glyphicon glyphicon-user"></span></a></td>
