...
<div class="portlet box yellow">
		<div class="portlet-title">
			<div class="caption">
				<i class="fa fa-gift"></i>Add New Rent
			</div>										
     		</div>
            <div class="portlet-body form">
										<!-- BEGIN FORM-->
										<form action="" method="POST" name="rent" class="form-horizontal">
	...
	*/
	...
	$token_id=md5(time());
$mysql_insert_rent = $dbhi->prepare("INSERT INTO `finance`.`rent` 
...
$mysql_insert_revenue_office->execute();
					...
					while($row = mysql_fetch_assoc($result))
						{
							...
							$q = mysql_query("SELECT SUM(amount) as sum FROM rent WHERE amount > 0") or die(mysql_error());
	...
	if(isset($_POST['print_button'])){
		...
		foreach($field as $key){
				...
				$rnt = datamanager::get_single('rent',array('id'=>$key),'rent');
				$amt= $rnt + $amt;
				echo $amt;
				datamanager::save(array('rent'=>$rnt),'revenue',array('id'=>1));
			}
		}
