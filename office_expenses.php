...
define("DB_PASSWORD","");
		...
		if(isset($_GET['delete']) && isset($_GET['confirm']))
				{
					$delete_office=$_GET['delete'];
					if($_GET['confirm'] == 'yes')
						{
							
							
							$sql5_revenue = $dbhi->prepare("SELECT token_id FROM office WHERE id='$delete_office'");
							...
							$sql5 = $dbhi->prepare("DELETE from office WHERE id=$delete_office");
							$sql5->execute();
							
							header("location:office_expenses.php?data_succesfully_deleted");
					...
					<div class="portlet box yellow">
						<div class="portlet-title">
							<div class="caption">
								...
								<i class="fa fa-user"></i>Office Expenses 
						...
						$xyz=datamanager::get_sum_tot('office','amount','pric');
						foreach ($xyz as $keyxyz);
						echo '<b>Total='.$keyxyz['pric'].'&nbsp;&nbsp;&nbsp;</b>';
						?>
						
						</div>
							<div align="right" style="padding:12px; font-weight:none; font-size:17px;"><a href="add_expenses.php">Add Office Expenses</a></div>
						</div>
...
$delete_office=$_GET['delete'];
{
							...
							$sql4 = $dbhi->prepare("SELECT * FROM  office WHERE id=$delete_office");
...
$get_edit_id=$_GET['edit'];
	header("location:edit_office.php?id=$get_edit_id");
?>
		...
		<form name="edit_office" id="edit_office" method="POST" novalidate="novalidate" action="<?php  echo $_SERVER['PHP_SELF']."?edit=".$key3['id'];?>">
<?php		


$get_edit=$_GET['edit'];
$sql2 = $dbhi->prepare("SELECT * FROM  office WHERE id=$get_edit");
	$sql2->execute();
						...
						?>	
							...
							<tr class="odd gradeX">
								...
								<td><input class="new_input_text" type="text" name="item" value="<?php 
								if($_POST['item'])
								{
								echo 'test';
								}
...
if(!isset($_GET['delete']) && !isset($_GET['edit']))
{
					...
					$sql2 = $dbhi->prepare("SELECT * FROM  office WHERE date BETWEEN '$from_date' AND '$to_date'  ORDER BY `id` DESC");
							}
							else
							{
							$sql2 = $dbhi->prepare("SELECT * FROM  office  ORDER BY `id` DESC");
							}		
							$sql2->execute();
