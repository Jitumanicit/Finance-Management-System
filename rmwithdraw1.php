...
<?php require_once(dirname(__FILE__).'/sidebar.php');?>	
		...
		<?php  if(isset($_GET['edit'])){
			...
			<div class="portlet box yellow">
									<div class="portlet-title">
										<div class="caption">
											<i class="fa fa-gift"></i>Add Remuneration
										</div>										
									</div>
									<div class="portlet-body form">
										...
										<form action="" method="POST" name="add_remuneration" class="form-horizontal">
											...
											<div class="form-body">
												<div class="form-group">
													<label class="col-md-3 control-label">Withdraw</label>
													<div class="col-md-4">
														<input type="text"  name="withdraw" class="form-control" placeholder="Enter Withdraw">
							...
							<button type="submit" class="btn green"	onclick="this.value='Submitting ..';this.disabled='disabled'; this.form.submit();">Submit</button>
														<!--<button type="button" class="btn default">Cancel</button>-->
														<a href="rm1.php" class="btn default">Cancel</a>
													</div>
												</div>
											</div>
										</form>
	...
	if(isset($_POST['name']) && isset($_POST['role']) && isset($_POST['date']))
	{ 
	$name=$_POST['name'];
	$role=$_POST['role'];
...
$mysql_insert_dealer = $dbhi->prepare("INSERT INTO `finance`.`rm1` 
(`name`, `role`, `withdraw`,`deposit`,`date`,`token_id`) 
VALUES 
('$name', '$role', '$withdraw','$deposit', '$date','$token_id')");
$mysql_insert_dealer->execute();


$particulars_type='Remuneration';
$mysql_insert_revenue_office = $dbhi->prepare("INSERT INTO `finance`.`revenue` 
(`particulars`,`debit`,`credit`,`time`,`token_id`) 
VALUES 
('$particulars_type','$withdraw','$deposit','$date','$token_id')");
$mysql_insert_revenue_office->execute();

	header("location:rm1.php");
