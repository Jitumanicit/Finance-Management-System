...
define("DB_PASSWORD","");
		...
		if(isset($_GET['delete']) && isset($_GET['confirm']))
				{
					$delete_reprocess2=$_GET['delete'];
					if($_GET['confirm'] == 'yes')
						{
						
						
						$sql5_revenue = $dbhi->prepare("SELECT token_id FROM reprocess2 WHERE id='$delete_reprocess2'");
							...
							$sql5 = $dbhi->prepare("DELETE from reprocess2 WHERE id=$delete_reprocess2");
							$sql5->execute();
							header("location:reprocess2.php?data_succesfully_deleted");
					...
					<div class="portlet box yellow">
						<div class="portlet-title">
							<div class="caption">
								...
								<i class="fa fa-user"></i>Reprocess Payment Account 
						...
						$xyz=datamanager::get_sum_tot('reprocess2','amount','pric');
						...
						?>
							...
							<div align="right" style="padding:12px; font-weight:none; font-size:17px;"><a href="add_reprocess2.php">Add Reprocess Payment</a></div>
						</div>
...
$delete_reprocess2=$_GET['delete'];
{
							...
							$sql4 = $dbhi->prepare("SELECT * FROM  reprocess2 WHERE id=$delete_reprocess2");
							...
							for($i=0; $key3 = $sql4->fetch(); $i++)
						{
							...
							<!--<td><input type="checkbox" class="checkboxes" value="<?php echo $key3['id'];?>"/></td>-->
								...
								<td><?php echo $key3['vehicle_no'];?></td>
...
$get_edit_id=$_GET['edit'];
	header("location:edit_reprocess2.php?id=$get_edit_id");
?>
		...
		<form name="edit_reprocess2" id="edit_reprocess2" method="POST" novalidate="novalidate" action="<?php  echo $_SERVER['PHP_SELF']."?edit=".$key3['id'];?>">
<?php		


$get_edit=$_GET['edit'];
$sql2 = $dbhi->prepare("SELECT * FROM reprocess2 WHERE id=$get_edit");
	$sql2->execute();
						...
						?>	
							...
							<tr class="odd gradeX">
								...
								echo $key3['name'];
								?>"/></td>
								<td><input class="new_input_text" type="date" name="date" value="<?php echo $key3['date'];?>"/></td>
                                <td><input class="new_input_text" type="text" name="amount" value="<?php echo $key3['amount'];?>"/></td>
								<td><input class="new_input_text" type="text" name="vehicle_no" value="<?php echo $key3['vehicle_no'];?>"/></td>								
								<td><input class="new_input_text" type="text" name="chasis_no" value="<?php echo $key3['chasis-no'];?>"/></td>
								<!-- <td> -->
									<td>
										<div class="btn-toolbar margin-bottom-10">
														<div class="btn-group btn-group-sm btn-group-solid">	
...
if(!isset($_GET['delete']) && !isset($_GET['edit']))
{
					...
					$sql2 = $dbhi->prepare("SELECT * FROM  reprocess2 WHERE date BETWEEN '$from_date' AND '$to_date'  ORDER BY `id` DESC");
							}
							else
							{
							$sql2 = $dbhi->prepare("SELECT * FROM  reprocess2 ORDER BY `id` DESC");
							}		
							$sql2->execute();
