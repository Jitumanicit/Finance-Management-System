...
<?php require_once(dirname(__FILE__).'/sidebar.php');?>	
	...
	$get_id=$_GET['id'];
$mysql_select_dealer = $dbhi2->prepare("SELECT * FROM `finance`.`collection` WHERE `id`=$get_id");
...
$db_dealer_value = $mysql_select_dealer->fetch();
			...
			<div class="portlet box yellow">
									<div class="portlet-title">
										<div class="caption">
											<i class="fa fa-gift"></i>Edit Collection Recive
										</div>										
									</div>
									<div class="portlet-body form">
										<!-- BEGIN FORM-->
...
<input value="<?php echo $db_dealer_value['name']; ?>" type="text" name="name" class="form-control" placeholder="Enter Name">														
											...
											<div class="form-body">
												<div class="form-group">
													...
													<div class="col-md-4">
<input value="<?php echo $db_dealer_value['date']; ?>" type="text"  name="date" class="form-control" placeholder="Enter Date">														
													</div>
												</div>
											</div>
                                            <div class="form-body">
												<div class="form-group">
													<label class="col-md-3 control-label">Amount Recive</label>
													<div class="col-md-4">
<input value="<?php echo $db_dealer_value['amount']; ?>" type="text"  name="amount" class="form-control" placeholder="Enter Amount Recive">														
													</div>
												</div>
											</div>
                                            <div class="form-body">
												<div class="form-group">
													...
													<div class="col-md-4">
<input value="<?php echo $db_dealer_value['vehicle_no']; ?>" type="text"  name="vehicle_no" class="form-control" placeholder="Enter Vehicle No">														
													</div>
												</div>
											</div>
...
<input value="<?php echo $db_dealer_value['client_name']; ?>" type="text"  name="client_name" class="form-control" placeholder="Enter Client Name">																									
									...
									<button type="submit" class="btn green"	onclick="this.value='Submitting ..';this.disabled='disabled'; this.form.submit();">Update</button>																		  									<!--<button type="button" class="btn default">Cancel</button>-->
														<a href="collection_recive.php" class="btn default">Cancel</a>
													</div>
												</div>
											</div>
										</form>
							...
							$revenue_office_exp_token=$db_dealer_value['token_id'];
	...
	*/
$mysql_update_dealer = $dbhi2->prepare("UPDATE `finance`.`collection` 
SET `name`='$name',
	`date`='$date',
	`amount`='$amount', 
...
$mysql_update_revenue_office_exp = $dbhi2->prepare("UPDATE `finance`.`revenue` 
SET 
	`credit`='$amount',
	`time`='$date' 
WHERE  `token_id`='$revenue_office_exp_token';");
	$mysql_update_revenue_office_exp->execute();	
	



							header("location:collection_recive.php?show_id=$get_id&update=data_sucessfully_updated");
