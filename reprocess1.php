...
define("DB_PASSWORD","");
		...
		if(isset($_GET['delete']) && isset($_GET['confirm']))
				{
					$delete_reprocess1=$_GET['delete'];
					if($_GET['confirm'] == 'yes')
						{
						
						$sql5_revenue = $dbhi->prepare("SELECT token_id FROM reprocess1 WHERE id='$delete_reprocess1'");
							...
							$sql5 = $dbhi->prepare("DELETE from reprocess1 WHERE id=$delete_reprocess1");
							$sql5->execute();
							header("location:reprocess1.php?data_succesfully_deleted");
					...
					<div class="portlet box yellow">
						<div class="portlet-title">
							<div class="caption">
								...
								<i class="fa fa-user"></i>Reprocess Receive Account 
						...
						$xyz=datamanager::get_sum_tot('reprocess1','amount','pric');
						...
						?>
							...
							<div align="right" style="padding:12px; font-weight:none; font-size:17px;"><a href="add_reprocess1.php">Add Receive Account</a></div>
						</div>
...
$delete_reprocess1=$_GET['delete'];
{
							...
							$sql4 = $dbhi->prepare("SELECT * FROM  reprocess1 WHERE id=$delete_reprocess1");
...
$get_edit_id=$_GET['edit'];
	header("location:edit_reprocess1.php?id=$get_edit_id");
?>
		...
		<form name="edit_reprocess1" id="edit_reprocess1" method="POST" novalidate="novalidate" action="<?php  echo $_SERVER['PHP_SELF']."?edit=".$key3['id'];?>">
<?php		


$get_edit=$_GET['edit'];
$sql2 = $dbhi->prepare("SELECT * FROM reprocess1 WHERE id=$get_edit");
	$sql2->execute();
...
if(!isset($_GET['delete']) && !isset($_GET['edit']))
{
					...
					$sql2 = $dbhi->prepare("SELECT * FROM  reprocess1 WHERE date BETWEEN '$from_date' AND '$to_date'  ORDER BY `id` DESC");
							}
							else
							{
							$sql2 = $dbhi->prepare("SELECT * FROM  reprocess1 ORDER BY `id` DESC");
							}		
							$sql2->execute();
