<?php 
	...
	$_ROLE_2 = array('admin' =>'Admin','accounts' =>'Accounts','cashier' =>'Cashier' );
										...
										<input type="hidden"  name="id" value="<?php echo $key['id'];?>">																			
											...
											<div class="form-group">
													...
													<select name="role" class="form-control">
														<?php foreach ($_ROLE_2 as $key_role_type => $value): ?>
<option value="<?php echo $key_role_type; ?>" <?php if(strtolower($key['role']) == $key_role_type) { echo 'selected="selected"'; }?>><?php echo $value;?></option>
														<?php endforeach;?>																
														</select>
													</div>
												</div>
	...
	$get_id=$_GET['id'];
	$mysql_select_dealer = $dbhi2->prepare("SELECT * FROM `finance`.`staff` WHERE `id`=$get_id");
...
<input value="<?php echo $db_dealer_value['name']; ?>" type="text" name="name" class="form-control" placeholder="Enter Name">														
											...
											<div class="form-body">
												<div class="form-group">
													<label class="col-md-3 control-label">Email</label>
													<div class="col-md-4">
<input value="<?php echo $db_dealer_value['email']; ?>" type="text"  name="email" class="form-control" placeholder="Enter Email">														
													</div>
												</div>
											</div>
											<div class="form-body">
...
<input value="<?php echo $db_dealer_value['phone']; ?>" type="text"  name="phone" class="form-control" placeholder="Enter Phone">														
											...
											<div class="form-body">
												<div class="form-group">
													...
													<select name="role" class="form-control">
														<?php foreach ($_ROLE_2 as $key_role_type => $value): ?>
<option value="<?php echo $key_role_type; ?>" <?php if(strtolower($db_dealer_value['role']) == $key_role_type) { echo 'selected="selected"'; }?>><?php echo $value;?></option>
														<?php endforeach;?>																
														</select>													
													</div>
												</div>
                                            ...
                                            <div class="form-body">
												<div class="form-group">
													<label class="col-md-3 control-label">Date of Joining</label>
													<div class="col-md-4">
<input value="<?php echo $db_dealer_value['doj']; ?>" type="date"  name="doj" class="form-control" placeholder="Enter Date">														
													</div>
												</div>
											</div>
                                  			...
                                  			<div class="form-body">
												<div class="form-group">
													<label class="col-md-3 control-label">Address</label>
													<div class="col-md-4">
<input value="<?php echo $db_dealer_value['address']; ?>" type="text"  name="address" class="form-control" placeholder="Enter Address">																									
													</div>
												</div>
											</div>
											<div class="form-actions fluid">
									...
									<button type="submit" class="btn green"	onclick="this.value='Submitting ..';this.disabled='disabled'; this.form.submit();">Update</button>																		  									<!--<button type="button" class="btn default">Cancel</button>-->
										...
										<?php
										if($_POST)
										$admin_staff_token=$db_dealer_value['token_id'];
	{
	if(isset($_POST['name'])  && isset($_POST['email']) && isset($_POST['phone']) && isset($_POST['role']) && isset($_POST['doj']) && isset($_POST['address']))
	{ 
	$name=$_POST['name'];
	$email=$_POST['email'];
	$phone=$_POST['phone'];
	...
	*/
$mysql_update_dealer = $dbhi2->prepare("UPDATE `finance`.`staff` 
SET `name`='$name',
	`email`='$email',
	`phone`='$phone', 
	`role`='$role', 
...
$mysql_update_admin = $dbhi2->prepare("UPDATE `finance`.`admin` 
SET `name`='$name',
	`email`='$email',
	`phone`='$phone', 
	`role`='$role', 
	`address`='$address'	
WHERE  `id`=$get_id;");
	$mysql_update_admin->execute();

							header("location:staff_list.php?show_id=$get_id&update=data_sucessfully_updated");
