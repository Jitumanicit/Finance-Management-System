...
<?php require_once(dirname(__FILE__).'/sidebar.php');?>	
	...
	$get_id=$_GET['id'];
$mysql_select_dealer = $dbhi2->prepare("SELECT * FROM `finance`.`office` WHERE `id`=$get_id");
...
$db_dealer_value = $mysql_select_dealer->fetch();
			...
			<div class="portlet box yellow">
									<div class="portlet-title">
										<div class="caption">
											<i class="fa fa-gift"></i>Edit Office Expenses
										</div>										
									</div>
									<div class="portlet-body form">
										<!-- BEGIN FORM-->
										<form action="" method="POST" name="add_staff" class="form-horizontal">
											...
											<div class="form-body">
												<div class="form-group">
													...
													<div class="col-md-4">
<input value="<?php echo $db_dealer_value['item']; ?>" type="text" name="item" class="form-control" placeholder="Enter Item">														
													</div>
												</div>
											</div>
											<div class="form-body">
...
<input value="<?php echo $db_dealer_value['date']; ?>" type="date"  name="date" class="form-control" placeholder="Enter Date">																									
									...
									<button type="submit" class="btn green"	onclick="this.value='Submitting ..';this.disabled='disabled'; this.form.submit();">Update</button>																		  									<!--<button type="button" class="btn default">Cancel</button>-->
														<a href="office_expenses.php" class="btn default">Cancel</a>
													</div>
												</div>
											</div>
										</form>
										...
										// echo $revenue_office_exp_token; echo "<br/>";
										// echo $db_dealer_value['token_id'];
										
										if($_POST)
	{
	if(isset($_POST['item']) && isset($_POST['amount']) && isset($_POST['date']))
	{ 
	$item=$_POST['item'];
	$amount=$_POST['amount'];
	$date=$_POST['date'];
	...
	*/
$mysql_update_dealer = $dbhi2->prepare("UPDATE `finance`.`office` 
SET `item`='$item',
	`amount`='$amount',
	`date`='$date' 
WHERE  `id`=$get_id;");
...
WHERE  `token_id`='$revenue_office_exp_token';");
	$mysql_update_revenue_office_exp->execute();
							...
							header("location:office_expenses.php?show_id=$get_id&update=data_sucessfully_updated");
